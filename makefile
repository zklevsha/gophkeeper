...
server:
	cd ./cmd/server/ && go build -v -ldflags="-X 'main.buildVersion=$(buildVersion)' -X 'main.buildDate=$(buildDate)' -X main.buildCommit=$(buildCommit)" .



client:
	cd ./cmd/client/ && go build  -v -ldflags="-X 'main.buildVersion=$(buildVersion)' -X 'main.buildDate=$(buildDate)' -X main.buildCommit=$(buildCommit)" .
